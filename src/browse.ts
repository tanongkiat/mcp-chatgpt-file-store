...
const BROWSE_PASSWORD =
  process.env.MCP_BROWSE_PASSWORD || process.env.MCP_AUTH_TOKEN || "";
...
export function getBrowseConfig(): BrowseConfig {
  return {
    user: BROWSE_USER,
...
export function checkCredentials(username: string, password: string): boolean {
  if (BROWSE_USER.length === 0 || BROWSE_PASSWORD.length === 0) return false;
...
function base64urlEncode(obj: SessionPayload): string {
  return Buffer.from(JSON.stringify(obj)).toString("base64url");
}

function base64urlDecode(raw: string): SessionPayload | undefined {
  try {
...
function verifyToken(raw: string): string | undefined {
...
export function getSessionUser(req: IncomingMessage): string | undefined {
...
export function issueSessionCookie(user: string): string {
...
function buildCrumbs(rel: string): string {
  if (!rel) return "";
...
export interface BrowseState {
  user: string;
  rel: string; // relative path currently shown ("" for root)
  entries: FileEntry[];
  error?: string;
}
 ...
 */
export function renderBrowsePage(state: BrowseState): string {
...
export async function listForRel(rel: string): Promise<FileEntry[]> {
  const roots = getDefaultRoots();
