...
export function createFileStoreServer() {
    ...
    const server = new McpServer({
        ...
        instructions: "Sandboxed local file store. All paths must stay inside the allowed directories. " +
    ...
    server.registerTool("list_allowed_directories", {
        ...
        description: "Returns the directories ChatGPT is allowed to read from and write to. Call this first.",
    }, async () => {
        return {
            content: [
                {
    ...
    server.registerTool("list_directory", {
        ...
        description: "Lists files and folders inside a directory within the allowed store.",
        inputSchema: {
    ...
    server.registerTool("read_file", {
        title: "Read file",
        description: "Reads the full contents of a text file inside the allowed file store.",
        inputSchema: {
    ...
    server.registerTool("search_files", {
        title: "Search files",
        description: "Searches the file store for files matching a pattern (supports * and ?).",
        inputSchema: {
    ...
    server.registerTool("write_file", {
        title: "Write file",
        description: "Creates or overwrites a file in the file store. Parent folders are created automatically. " +
        ...
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ ok: true, action: "wrote", path: info.path, size: info.size }, null, 2),
                },
            ],
        };
    });
    server.registerTool("append_file", {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ ok: true, action: "appended", path: info.path, size: info.size }, null, 2),
                },
            ],
        };
    });
    server.registerTool("create_directory", {
        ...
        return {
            content: [
                {
                    type: "text",
                    text: JSON.stringify({ ok: true, action: "created", path: info.path }, null, 2),
                },
            ],
        };
    });
    ...
    server.registerTool("delete_file", {
        ...
        description: "Permanently deletes a file or folder (recursively) from the file store. Use with care.",
        inputSchema: {
