 ...
 */
export async function gatherKnowledge(roots, options = {}) {
...
export function renderMarkdown(result) {
    ...
    const sections = result.docs.map((doc) => `## ${doc.relPath}\n\n_Last modified: ${doc.modified}_\n\n${doc.content.trim()}`);
 ...
 */
export function extractFlows(doc) {
    const flows = [];
...
export function renderPlantUml(result) {
    ...
    const blocks = flows.map((flow) => `### ${flow.relPath} (${flow.origin})\n\n\`\`\`plantuml\n${flow.uml}\n\`\`\``);
 ...
 */
export function renderAuto(result) {
